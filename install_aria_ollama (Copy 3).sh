  ...
  <main class="shell">
    <section class="app">
      <section class="hero">
        <div class="hero-card">
          <div class="topbar">
            <button id="settingsBtn" class="settings-button" aria-label="Settings">⚙</button>
          </div>
          <div class="hero-top">
            <div>
          ...
          <div class="status-grid">
            <div class="stat">
              ...
              <div id="wakeState" class="stat-value">Ready</div>
            </div>
          </div>
        </div>
        <div class="panel" style="padding:24px;">
          <div class="command-bar">
            <div class="command-group">
              ...
              <button id="wakeBtn" class="mic-button">🎙 On</button>
              <button id="speakBtn">🔊 On</button>
      ...
      <section class="tool-hub">
        <section class="tool-panel">
          <h3 class="section-title">Tool Activity</h3>
          <div id="toolTimeline" class="timeline">
            <div class="hint">No tool activity yet.</div>
          </div>
        </section>
        <section class="tool-panel">
          <h3 class="section-title">Results</h3>
          <div id="resultCards" class="result-grid">
            <div class="hint">Searches, file previews, and opened targets will appear here.</div>
          </div>
        </section>
      </section>
  ...
  <div id="settingsOverlay" class="overlay">
    <section class="modal">
      ...
      <div class="modal-grid">
        ...
        <div class="stack">
          <div class="modal-card">
            <div class="label">Current Brain</div>
          ...
          <div id="settingsNotice" class="notice"><strong>Ready.</strong> Choose a model to use, or keep Smart Auto.</div>
        </div>
      </div>
    </section>
  </div>
  ...
  <script>
    ...
    function renderCurrentBrain(active) {
      ...
      rows.forEach((text) => {
        ...
        row.innerHTML = `<span class="status-dot ${active.status === "Running" || active.status === "Connected" ? "" : ""}"></span><span>${escapeHtml(text)}</span>`;
      ...
      models.forEach((model) => {
        ...
        option.onclick = async () => {
          if (model.id === "auto") {
            ...
            showSettingsNotice("Smart Auto will choose the best model for each task.", "success");
            return;
          }
    ...
    function initRecognition() {
      ...
      wakeRecognition.onstart = () => {
        if (!commandCaptureActive) setStatus("Ready", "armed");
      };
      ...
      commandRecognition.onstart = () => setStatus("Listening", "listening");
      ...
      wakeBtn.onclick = async () => {
        ...
        wakeBtn.textContent = `🎙 ${wakeWordEnabled ? "On" : "Off"}`;
        wakeStateEl.textContent = wakeWordEnabled ? "Ready" : "Sleeping";
        stopAllRecognition();
        if (wakeWordEnabled) {
          if (!(await ensureMic())) return;
