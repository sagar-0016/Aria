...
configure_package_manager() {
  if have_cmd apt-get; then
    PKG_MANAGER="apt"
    INSTALL_CMD=(${SUDO:+$SUDO} apt-get install -y --no-install-recommends)
    UPDATE_CMD=(${SUDO:+$SUDO} apt-get update)
    PKG_QUERY_CMD=(dpkg-query -W -f='${db:Status-Status}')
    APT_CLEANUP_CMD=(${SUDO:+$SUDO} apt-get autoremove -y)
...
package_installed() {
  case "${PKG_MANAGER}" in
    apt)
      [[ "$("${PKG_QUERY_CMD[@]}" "$1" 2>/dev/null)" == "installed" ]]
      ;;
    *)
...
cleanup_package_cache() {
  ...
  log "[2b/9] Trimming Debian package cache..."
  run_logged "${APT_CLEANUP_CMD[@]}" || true
...
WAKE_WORDS_PATH = ARIA_DIR / "wake_words.txt"
USER_IMPROVEMENTS_PATH = ARIA_DIR / "user_improvements.json"
...
PYEOF
  ...
  cat >"${ARIA_DIR}/improvements_manager.py" <<'PYEOF'
import json
from pathlib import Path

from config import DEFAULT_IMPROVEMENTS, USER_IMPROVEMENTS_PATH


class ImprovementsManager:
    def __init__(self):
        self.path = Path(USER_IMPROVEMENTS_PATH)
        self.path.parent.mkdir(parents=True, exist_ok=True)
        self._data = self._load()

    def _load(self):
        if not self.path.exists():
            self.save(DEFAULT_IMPROVEMENTS.copy())
        try:
            with self.path.open("r", encoding="utf-8") as handle:
                data = json.load(handle)
        except Exception:
            data = {}
        merged = {
            "wake_words": list(DEFAULT_IMPROVEMENTS["wake_words"]),
            "speech_fixes": dict(DEFAULT_IMPROVEMENTS["speech_fixes"]),
            "command_aliases": dict(DEFAULT_IMPROVEMENTS["command_aliases"]),
        ...
        for key in ["wake_words", "speech_fixes", "command_aliases", "routing_hints", "tool_preferences", "prompt_append"]:
            if key in data:
                ...
                elif isinstance(merged[key], list) and isinstance(data[key], list):
                    merged[key] = data[key]
                else:
                    merged[key] = data[key]
        return merged
    ...
    def update(self, patch):
        ...
        if "wake_words" in patch and isinstance(patch["wake_words"], list):
...
class BrowserTools:
    ...
    def play_media(self, query, source="youtube music"):
        cleaned = (query or "").strip()
        if not cleaned:
            return {"ok": False, "message": "Tell me what you want to play.", "items": [], "execution": {"kind": "query", "value": ""}}
        source_name = (source or "youtube music").strip().lower()
        if source_name in {"youtube music", "yt music", "music"}:
            url = f"https://music.youtube.com/search?q={cleaned.replace(' ', '+')}"
            label = "YouTube Music"
        else:
            url = f"https://www.youtube.com/results?search_query={cleaned.replace(' ', '+')}"
            label = "YouTube"
        webbrowser.open(url)
        return {"ok": True, "message": f"Opened {label} for {cleaned}.", "items": [{"title": cleaned, "url": url, "snippet": f"Opened in {label}."}], "execution": {"kind": "url", "value": url}}
PYEOF
...
class FileTools:
    ...
    def count_files(self, raw_path="."):
        target = self._resolve(raw_path or ".")
        if not target.exists() or not target.is_dir():
            return {"ok": False, "message": "That folder does not exist.", "items": [], "execution": {"kind": "path", "value": str(target)}}
        count = sum(1 for child in target.iterdir() if child.is_file())
        return {"ok": True, "message": f"There are {count} files in {target}.", "items": [{"title": target.name, "url": str(target), "snippet": f"{count} files"}], "execution": {"kind": "path", "value": str(target)}}

    def recent_file(self, raw_path="."):
        base = self._resolve(raw_path or ".")
        if not base.exists():
            return {"ok": False, "message": "That location does not exist.", "items": [], "execution": {"kind": "path", "value": str(base)}}
        if base.is_file():
            candidate = base
        else:
            files = [child for child in base.rglob("*") if child.is_file()]
            if not files:
                return {"ok": False, "message": "I could not find any files there.", "items": [], "execution": {"kind": "path", "value": str(base)}}
            candidate = max(files, key=lambda item: item.stat().st_mtime)
        return {
            "ok": True,
            "message": f"Most recent file: {candidate.name} at {candidate}",
            "items": [{"title": candidate.name, "url": str(candidate), "snippet": "Most recently modified file"}],
            "execution": {"kind": "path", "value": str(candidate)},
        }
PYEOF

  cat >"${ARIA_DIR}/tools/clipboard_tools.py" <<'PYEOF'
import shutil
import subprocess
...
class ClipboardTools:
    ...
    def read_text(self):
        ...
        if not candidates:
            return {"ok": False, "message": "Clipboard access is not available. Install wl-clipboard or xclip.", "items": [], "execution": {"kind": "backend", "value": "missing"}}
        for command in candidates:
            ok, text = self._run(command)
            if ok:
                cleaned = text.strip()
                if cleaned:
                    return {"ok": True, "message": cleaned[:2000], "items": [{"title": "Clipboard", "url": "", "snippet": cleaned[:160]}], "execution": {"kind": "command", "value": command}}
                return {"ok": True, "message": "Your clipboard is empty.", "items": [], "execution": {"kind": "command", "value": command}}
        return {"ok": False, "message": "I could not read the clipboard.", "items": [], "execution": {"kind": "command", "value": candidates[0]}}
PYEOF
...
PYEOF
  ...
  cat >"${ARIA_DIR}/tools/desktop_input_tools.py" <<'PYEOF'
import re
import shutil
import subprocess


class DesktopInputTools:
    MODIFIER_KEYS = {
        "ctrl": ("ctrl", "Control_L"),
    ...
    KEY_ALIASES = {
        ...
        "windows": ("Super_L", "Super_L"),
        "win": ("Super_L", "Super_L"),
        "meta": ("Super_L", "Super_L"),
        "tv": ("t", "t"),
        "ti": ("t", "t"),
        "tea": ("t", "t"),
        "tee": ("t", "t"),
    ...
    def _backend_hint(self):
        return "Install xdotool, or install wtype on Wayland-based desktops."
    ...
    def _tokenize(self, raw):
        ...
        tokens = [token for token in lowered.split() if token not in {"press", "pressed", "key", "button", "the", "command", "ki"}]
        if tokens[:2] == ["caps", "lock"]:
            tokens = ["capslock"] + tokens[2:]
        if tokens[:2] == ["page", "down"]:
            tokens = ["pagedown"] + tokens[2:]
        if tokens[:2] == ["page", "up"]:
            tokens = ["pageup"] + tokens[2:]
        if len(tokens) >= 2 and tokens[-1] == "arrow" and tokens[-2] in {"down", "up", "left", "right"}:
            tokens = tokens[:-1]
        return tokens
    ...
    def _normalize_key(self, token):
        if not token:
            return None
        if token in self.MODIFIER_KEYS:
            return {"kind": "modifier", "wtype": self.MODIFIER_KEYS[token][0], "xdotool": self.MODIFIER_KEYS[token][1]}
        if token in self.KEY_ALIASES:
            return {"kind": "key", "wtype": self.KEY_ALIASES[token][0], "xdotool": self.KEY_ALIASES[token][1]}
        if re.fullmatch(r"f([1-9]|1[0-2])", token):
            return {"kind": "key", "wtype": token.upper(), "xdotool": token.upper()}
        if len(token) == 1 and token.isalnum():
            return {"kind": "key", "wtype": token.lower(), "xdotool": token.lower()}
        return None

    def _parse_combo(self, raw):
        ...
        for token in tokens:
            if token == "+":
                continue
            item = self._normalize_key(token)
            if not item:
                return [], f"I do not know how to press '{token}' safely."
    ...
    def press_key(self, key_name):
        combo, error = self._parse_combo(key_name)
        if error:
            return {"ok": False, "message": error, "items": [], "execution": {"kind": "input", "value": key_name}}
        if len(combo) != 1:
            return self.press_hotkey(key_name)
        backend = self._backend()
        if not backend:
            return {"ok": False, "message": f"Desktop input is not available here. {self._backend_hint()}", "items": [], "execution": {"kind": "backend", "value": "missing"}}
        key = combo[0]
        if backend == "xdotool":
            execution = self._run_command(["xdotool", "key", key["xdotool"]])
        else:
            execution = self._run_command(["wtype", "-k", key["wtype"]])
        pretty = self._pretty_combo(combo) or "key"
        return {"ok": True, "message": f"Pressed {pretty}.", "items": [], "execution": execution}

    def press_hotkey(self, hotkey):
        combo, error = self._parse_combo(hotkey)
        if error:
            return {"ok": False, "message": error, "items": [], "execution": {"kind": "input", "value": hotkey}}
        if len(combo) < 2:
            return self.press_key(hotkey)
        backend = self._backend()
        if not backend:
            return {"ok": False, "message": f"Desktop input is not available here. {self._backend_hint()}", "items": [], "execution": {"kind": "backend", "value": "missing"}}
        modifiers = combo[:-1]
        final_key = combo[-1]
        if any(item["kind"] != "modifier" for item in modifiers) or final_key["kind"] != "key":
            return {"ok": False, "message": "Use hotkeys like ctrl+t, alt+tab, or super.", "items": [], "execution": {"kind": "input", "value": hotkey}}
        if backend == "xdotool":
            execution = self._run_command(["xdotool", "key", "+".join([item["xdotool"] for item in modifiers] + [final_key["xdotool"]])])
        else:
            command = ["wtype"]
            for item in modifiers:
                command += ["-M", item["wtype"]]
            command += ["-k", final_key["wtype"]]
            for item in reversed(modifiers):
                command += ["-m", item["wtype"]]
            execution = self._run_command(command)
        pretty = self._pretty_combo(combo)
        return {"ok": True, "message": f"Pressed {pretty}.", "items": [], "execution": execution}

    def type_text(self, text):
        cleaned = (text or "").strip()
        if not cleaned:
            return {"ok": False, "message": "I need some text to type.", "items": [], "execution": {"kind": "input", "value": ""}}
        backend = self._backend()
        if not backend:
            return {"ok": False, "message": f"Desktop input is not available here. {self._backend_hint()}", "items": [], "execution": {"kind": "backend", "value": "missing"}}
        if backend == "xdotool":
            execution = self._run_command(["xdotool", "type", "--delay", "12", cleaned])
        else:
            execution = self._run_command(["wtype", cleaned])
        return {"ok": True, "message": f"Typed: {cleaned}", "items": [], "execution": execution}
PYEOF
...
from tools.desktop_input_tools import DesktopInputTools
...
class ToolExecutor:
    def __init__(self):
        ...
        self.desktop_input = DesktopInputTools()
    ...
    def available_tools(self):
        return {
            ...
            "clipboard": "Read the current clipboard text when available.",
            "desktop_input": "Press safe keys, trigger safe hotkeys, and type text using the desktop input backend.",
        ...
        return {
            "browser": {
                "actions": {
                    ...
                    "play_media": {"query": "song or media request", "source": "youtube music|youtube"},
                    "search_site": {"site": "youtube|youtube music|google|github", "query": "search query string"},
                }
            },
            "clipboard": {
                "actions": {
            ...
            "desktop_input": {
                "actions": {
                    "press_key": {"key": "super|tab|enter|escape|letters|function keys"},
                    "press_hotkey": {"hotkey": "ctrl+t|alt+tab|super"},
                    "type_text": {"text": "text to type into the active window"},
                }
            },
            "system": {
                "actions": {
            ...
            "files": {
                "actions": {
                    ...
                    "count_files": {"path": "relative or home-absolute path"},
                    "recent_file": {"path": "relative or home-absolute path"},
                }
            },
            "search": {
                "actions": {
    ...
    def prompt_block(self):
        return (
            ...
            "Use action='play_media' for music or play requests that should open YouTube Music or YouTube. "
            "Use action='search_site' when the user wants a site opened and searched in one step, such as YouTube plus a query. "
            "For keyboard requests, use tool='desktop_input' with action='press_key', 'press_hotkey', or 'type_text'. "
    ...
    def _normalize_call(self, call):
        ...
        if tool_name in {"play", "media", "music"}:
            payload["tool"] = "browser"
            payload["action"] = "play_media"
            payload["query"] = str(payload.get("query") or payload.get("title") or payload.get("target") or payload.get("url") or "").strip()
            payload["source"] = str(payload.get("source") or "youtube music").strip()
            payload.pop("target", None)
            payload.pop("url", None)
            return payload

        if tool_name == "browser" and action in {"search", "find"}:
            payload["action"] = "search_site"
            payload["site"] = str(payload.get("site") or "google").strip()
            payload["query"] = str(payload.get("query") or payload.get("target") or "").strip()
            payload.pop("target", None)
            return payload
        ...
        if tool_name == "browser" and action in {"play", "listen"}:
            payload["action"] = "play_media"
            payload["query"] = str(payload.get("query") or payload.get("target") or payload.get("url") or "").strip()
            payload["source"] = str(payload.get("source") or "youtube music").strip()
            payload.pop("target", None)
            payload.pop("url", None)
            return payload

        if tool_name in {"keyboard", "keys", "input"}:
            payload["tool"] = "desktop_input"
            if action in {"press", "tap", "key"}:
                value = str(payload.get("hotkey") or payload.get("key") or payload.get("target") or "").strip()
                payload["action"] = "press_hotkey" if any(separator in value for separator in ["+", " ", "-"]) else "press_key"
                if payload["action"] == "press_hotkey":
                    payload["hotkey"] = value
                ...
                payload["text"] = str(payload.get("text") or payload.get("query") or payload.get("value") or "").strip()
            return payload
        ...
        try:
            if tool_name == "browser":
                if action == "open_target":
                    ...
                    result = self.browser.play_media(str(payload.get("query", "")).strip(), str(payload.get("source", "youtube music")).strip())
                elif action == "search_site":
                    site = str(payload.get("site", "google")).strip().lower()
                    query = str(payload.get("query", "")).strip()
                    if site in {"youtube", "youtube music", "yt music"}:
                        result = self.browser.play_media(query, site)
                    else:
                        result = self.browser.search_url(f"site:{site} {query}".strip())
                else:
                    result = {"ok": False, "message": "That browser action is not available.", "items": []}
            elif tool_name == "clipboard":
                if action == "read_text":
                    result = self.clipboard.read_text()
                else:
                    result = {"ok": False, "message": "That clipboard action is not available.", "items": []}
            elif tool_name == "desktop_input":
                if action == "press_key":
                    result = self.desktop_input.press_key(str(payload.get("key", "")).strip())
                elif action == "press_hotkey":
                    result = self.desktop_input.press_hotkey(str(payload.get("hotkey", "")).strip())
                elif action == "type_text":
                    result = self.desktop_input.type_text(str(payload.get("text", "")).strip())
                else:
                    result = {"ok": False, "message": "That desktop input action is not available.", "items": []}
                ...
                if action == "list_dir":
                    ...
                    result = self.files.count_files(path or ".")
                elif action == "recent_file":
                    result = self.files.recent_file(path or ".")
                else:
    ...
    def route(self, text):
        ...
        if "what is in my clipboard" in lowered or "what is on my clipboard" in lowered or "read my clipboard" in lowered:
            return self.execute_call({"tool": "clipboard", "action": "read_text"})
        if "what is my most recent file" in lowered or "what is my latest file" in lowered or "what is my newest file" in lowered or "what is my most recent five" in lowered:
            return self.execute_call({"tool": "files", "action": "recent_file", "path": "."})
        if "how many files" in lowered and "downloads" in lowered:
            return self.execute_call({"tool": "files", "action": "count_files", "path": "Downloads"})
        ...
        match = re.match(r"^open\s+youtube(?:\s+and)?\s+play\s+(.+)$", lowered)
        if match:
            return self.execute_call({"tool": "browser", "action": "play_media", "query": match.group(1).strip(), "source": "youtube"})
        ...
        match = re.match(r"^play\s+(.+?)(?:\s+in\s+(youtube music|yt music|youtube))?$", lowered)
        if match:
            query = match.group(1).strip()
            source = (match.group(2) or "youtube music").strip()
            return self.execute_call({"tool": "browser", "action": "play_media", "query": query, "source": source})
        if re.match(r"^(press|hit|tap)\s+", lowered):
            phrase = re.sub(r"^(press|hit|tap)\s+", "", lowered).strip()
            phrase = phrase.replace("windows key", "super").replace("super key", "super").replace("windows button", "super")
            phrase = phrase.replace("control ", "ctrl+").replace("ctrl ", "ctrl+").replace("alt ", "alt+").replace("shift ", "shift+")
            phrase = phrase.replace("super ", "super+")
            phrase = re.sub(r"\++", "+", phrase).strip("+")
            if "+" in phrase:
                return self.execute_call({"tool": "desktop_input", "action": "press_hotkey", "hotkey": phrase})
            return self.execute_call({"tool": "desktop_input", "action": "press_key", "key": phrase})
        match = re.match(r"^(?:type|write)\s+(.+)", text, re.IGNORECASE)
        if match:
            return self.execute_call({"tool": "desktop_input", "action": "type_text", "text": match.group(1).strip()})
        ...
        if "most recent file" in lowered or "latest file" in lowered or "newest file" in lowered:
            return self.execute_call({"tool": "files", "action": "recent_file", "path": "."})
...
from config import HOST, INDEX_HTML, OLLAMA_HOST, PORT, SUPPORTED_MODELS, WAKE_WORDS_PATH
from improvements_manager import ImprovementsManager
...
TOOL_SYSTEM_PROMPT = (
    ...
    "Prefer tools for internet lookups, opening desktop targets, safe system questions, home-directory file tasks, and desktop input actions like key presses or typing. "
    "Do not invent tool names or actions. "
    "Never claim a key was pressed or text was typed unless a desktop_input tool returned success. "
...
def load_wake_words():
    try:
        words = [line.strip().lower() for line in WAKE_WORDS_PATH.read_text(encoding="utf-8").splitlines()]
    except Exception:
        words = []
    cleaned = []
    for word in words:
        if word and word not in cleaned:
            cleaned.append(word)
    for word in IMPROVEMENTS.get().get("wake_words", []):
...
def normalize_user_text(text):
    ...
    for source, replacement in improvements.get("speech_fixes", {}).items():
        if not source:
            continue
        normalized = re.sub(rf"\b{re.escape(str(source))}\b", str(replacement), normalized, flags=re.IGNORECASE)
    lowered = normalized.lower().strip()
    for source, replacement in improvements.get("command_aliases", {}).items():
...
def build_messages(text):
    ...
    prompt_append = str(IMPROVEMENTS.get().get("prompt_append", "")).strip()
    if prompt_append:
        prompt = f"{prompt} {prompt_append}"
...
def update_references_from_events(tool_events):
    for event in tool_events:
        result = event.get("result", {})
        ...
        if not items:
            continue
        first = items[0]
        if event.get("tool") == "files":
            LAST_REFERENCES["file"] = {
                "title": str(first.get("title", "")),
                "path": str(first.get("url", "")),
            }


def direct_reference_reply(text):
    lowered = text.lower().strip()
    if "location of that file" in lowered or "where is that file" in lowered or "path of that file" in lowered:
        file_ref = LAST_REFERENCES.get("file")
        if file_ref and file_ref.get("path"):
            return f"{file_ref.get('title') or 'That file'} is at {file_ref['path']}."
    return ""


def execute_text_steps(text):
    segments = [part.strip() for part in re.split(r"\b(?:and then|then|after that)\b", text, flags=re.IGNORECASE) if part.strip()]
    if len(segments) < 2:
        return []
    events = []
    for segment in segments[:4]:
        event = TOOLS.route(segment)
        if not event.get("used"):
            return []
        events.append(event)
    return events


def compose_tool_reply(tool_events):
    if not tool_events:
        return ""
    parts = []
    for event in tool_events:
        message = str(event.get("result", {}).get("message", "")).strip()
        if message:
            parts.append(message)
    return " Then ".join(parts)


def should_reply_with_tool_result(user_text, tool_events):
    if not tool_events:
        return False
    if any(not event.get("result", {}).get("ok") for event in tool_events):
        return False
    message = compose_tool_reply(tool_events)
    ...
    if tool_name in {"browser", "system", "files", "clipboard"} and (
        ...
        or "recent file" in lowered
        or "latest file" in lowered
        or "what time" in lowered
        or "what date" in lowered
...
class AriaHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/api/improvements":
            self._send_json({"ok": True, "improvements": IMPROVEMENTS.get(), "status": status_payload()})
            return
    ...
    def do_POST(self):
        ...
        if self.path == "/api/chat":
            ...
            if text.lower() == "play":
                ...
                return
            reference_reply = direct_reference_reply(text)
            if reference_reply:
                active = active_model_snapshot()
                CHAT_HISTORY.extend(
                    [
                        {"role": "user", "content": text},
                        {"role": "assistant", "content": reference_reply},
                    ]
                )
                log_interaction(text, reference_reply, active["active_model"], [])
                self._send_json(
                    {
                ...
                return
            tool_events = []
            step_events = execute_text_steps(text)
            if step_events:
                tool_events = step_events
            else:
            ...
            if should_reply_with_tool_result(text, tool_events):
                reply = compose_tool_reply(tool_events)
            CHAT_HISTORY.extend(
                [
                    {"role": "user", "content": text},
                    {"role": "assistant", "content": reply},
            ...
            update_references_from_events(tool_events)
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    body[data-state="listening"] .voice-state-dot {
      background: var(--accent);
      box-shadow: 0 0 22px rgba(0, 143, 17, 0.56), 0 0 42px rgba(0, 143, 17, 0.28);
    }
    ...
    .label { margin-bottom: 8px; font-size: 0.82rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.12em; }
    .api-field {
      display:grid;
      gap: 8px;
    }
...
<body data-state="idle" data-theme="light">
  ...
  <script>
    ...
    function updateWakeWords(words = []) {
      wakeWords = [...new Set((words || []).map((word) => String(word || "").trim().toLowerCase()).filter(Boolean))];
      if (!wakeWords.length) {
    ...
    async function sendText(text) {
      ...
      try {
        ...
        if (!data.stop_speaking && speakReplies && "speechSynthesis" in window) {
          window.speechSynthesis.cancel();
    ...
    function startWakeRecognition() {
      if (sleepMode || !wakeRecognition || wakeListeningActive) return;
      try {
        wakeRecognition.start();
      } catch (error) {
        wakeStartFailures += 1;
        if (wakeStartFailures > 2) {
    ...
    function initRecognition() {
      ...
      commandRecognition.onerror = (event) => {
        ...
        if (wakeWordEnabled && !assistantBusy) scheduleWakeRestart();
      };
    ...
    (async () => {
      ...
      if (SpeechRecognition) {
        ...
        if (ok && wakeWordEnabled) {
          setTimeout(() => {
            if (wakeWordEnabled && !sleepMode) scheduleWakeRestart();
          }, 700);
        }
      }
    })();
...
EOF
  ...
  chown "${TARGET_USER}:${TARGET_GROUP}" \
    ...
    "${ARIA_DIR}/improvements_manager.py" \
    "${ARIA_DIR}/router.py" \
    "${ARIA_DIR}/tool_executor.py" \
    "${ARIA_DIR}/aria.py" \
    "${ARIA_DIR}/wake_words.txt" \
    "${ARIA_DIR}/user_improvements.json" \
    ...
    "${ARIA_DIR}/tools/clipboard_tools.py" \
    "${ARIA_DIR}/tools/desktop_input_tools.py" \
...
enable_service() {
  ...
  if [[ ${EUID} -eq 0 ]] && have_cmd loginctl; then
    run_logged loginctl enable-linger "${TARGET_USER}" || true
  fi

  if [[ -d "${runtime_dir}" ]]; then
    run_logged run_as_target env XDG_RUNTIME_DIR="${runtime_dir}" systemctl --user daemon-reload || true
    run_logged run_as_target env XDG_RUNTIME_DIR="${runtime_dir}" systemctl --user enable ollama.service || true
    run_logged run_as_target env XDG_RUNTIME_DIR="${runtime_dir}" systemctl --user restart ollama.service || true
    if run_as_target env XDG_RUNTIME_DIR="${runtime_dir}" systemctl --user is-active --quiet ollama.service; then
      printf "  Ollama user service is active.\n"
      return
    fi
  fi

  warn "Could not confirm the Ollama user service is active. See ${INSTALL_LOG}."
}
...
configure_shell_path() {
  ...
  for profile in "${shell_profiles[@]}"; do
    if [[ ! -f "${profile}" ]]; then
      install -m 0644 -o "${TARGET_USER}" -g "${TARGET_GROUP}" /dev/null "${profile}"
    fi
