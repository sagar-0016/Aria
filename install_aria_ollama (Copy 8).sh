...
class DesktopInputTools:
    ...
    def _tokenize(self, raw):
        ...
        tokens = [token for token in lowered.split() if token not in {"press", "pressed", "key", "button", "the", "command"}]
        return tokens
    ...
    def press_key(self, key_name):
        ...
        pretty = key_name.strip().lower() or "key"
    ...
    def press_hotkey(self, hotkey):
        ...
        pretty = hotkey.strip().lower().replace("+", " + ")
        return {"ok": True, "message": f"Pressed {pretty}.", "items": [], "execution": execution}
...
class ToolExecutor:
    ...
    def route(self, text):
        ...
        match = re.match(r"^(?:type|write)\s+(.+)", lowered)
        if match:
...
def should_reply_with_tool_result(user_text, tool_events):
    ...
    if tool_name == "desktop_input":
        return True
    if tool_name in {"browser", "system", "files"} and (
